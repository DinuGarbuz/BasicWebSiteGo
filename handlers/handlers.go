...
func ShowUser(w http.ResponseWriter, r *http.Request) {
	http.ServeFile(w, r, "templates/showPlayers.html")
}
...
func ShowUserFunc(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		t, _ := template.ParseFiles("templates/showPlayersPage.html")
		t.Execute(w, nil)
		...
		for _, usr := range alUsrs.Users {
			...
			if usr.Id != id {
				...
				t, err := template.ParseFiles("templates/showPlayersPage.html")
				checkError(err)
				t.Execute(w, usr)
			}
...
func DeleteUserServe(w http.ResponseWriter, r *http.Request) {
	http.ServeFile(w, r, "templates/deletePlayer.html")
...
func DeleteUserFunc(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		t, _ := template.ParseFiles("templates/deletePlayer.html")
		t.Execute(w, nil)
	} else {
		r.ParseForm()
		id, err := strconv.Atoi(r.FormValue("id"))
...
func AddNewUserFunc(w http.ResponseWriter, r *http.Request) {
	...
	newUser := &model.User{}
	if r.Method == "GET" {
		t, _ := template.ParseFiles("templates/addNewPlayer.html")
		t.Execute(w, nil)
		...
		var err error
		newUser.Age, err = strconv.ParseFloat(r.FormValue("age"), 64)
		newUser.Image = r.FormValue("image")
		checkError(err)
